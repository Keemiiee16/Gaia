...
temperaments = ["calm", "fussy", "clingy", "dramatic", "sleepy", "curious", "playful", "sensitive"]

events = {
    "school": ["A rumor starts spreading in the hallway."],
    "celebrity": ["A fan gets too close and starts asking personal questions."],
    "medical": ["The doctor walks in with a serious expression."],
    "romance": ["Their partner notices something is wrong."],
    "family": ["A family member walks in at the worst time."],
    "fantasy": ["The air suddenly feels strange, like magic is nearby."],
    "default": ["Something unexpected happens."]
}

baby_events = {
    "calm": ["The baby softly whimpers but stays mostly calm."],
    "fussy": ["The baby starts crying hard and kicking their little legs."],
    "clingy": ["The baby cries the second they are put down."],
    "dramatic": ["The baby gives one tiny whine, then completely loses it."],
    "sleepy": ["The baby rubs their face and fights sleep."],
    "curious": ["The baby reaches for something and fusses when they cannot grab it."],
    "playful": ["The baby babbles happily and wants attention."],
    "sensitive": ["The baby startles and begins crying softly."]
}
...
async def character_list(interaction: discord.Interaction):
    ...
    msg = "\n".join(
        [f"- **{c['name']}** | {c['age']} | {c['gender']} | {c['genre']}" for c in u["characters"].values()]
    )
...
async def baby_event(interaction: discord.Interaction, name: str):
    ...
    await interaction.response.send_message(
        f"**{b['name']}**\n\n{random.choice(baby_events[b['temperament']])}"
    )
...
async def baby_reveal(interaction: discord.Interaction, name: str):
    ...
    await interaction.response.send_message(
        f"👀 **{b['name']}** temperament is **{b['temperament']}**."
    )
