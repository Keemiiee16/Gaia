...
GENRE_SELECT_OPTIONS = [
    discord.SelectOption(label="Slice of Life", value="slice_of_life", description="Everyday life, family, friendship, and emotional moments."),
    discord.SelectOption(label="School Drama", value="school_drama", description="Classes, rumors, hallway tension, exams, and teen drama."),
    discord.SelectOption(label="Celebrity Drama", value="celebrity_drama", description="Fans, fame, paparazzi, interviews, scandals, and viral moments."),
    discord.SelectOption(label="Romance", value="romance", description="Crushes, relationships, jealousy, breakups, and proposals."),
    discord.SelectOption(label="Family Drama", value="family_drama", description="Parents, siblings, confrontations, secrets, and support."),
    discord.SelectOption(label="Medical Drama", value="medical_drama", description="Hospitals, checkups, pregnancy scares, injuries, and recovery."),
    discord.SelectOption(label="Fantasy", value="fantasy", description="Magic, realms, supernatural storylines, and mythical drama."),
    discord.SelectOption(label="Chaotic RP", value="chaotic_rp", description="Messy, dramatic, random, high-energy roleplay twists."),
]
...
class CharacterCreateModal(discord.ui.Modal, title="Create Character"):
    def __init__(self, genres: List[str]):
        ...
        self.character_age = discord.ui.TextInput(
            label="Age",
            placeholder="Character age",
            max_length=20,
            required=True,
        )
        self.character_gender = discord.ui.TextInput(
            label="Gender",
            placeholder="Character gender",
            max_length=80,
            required=True,
        )
        self.character_appearance = discord.ui.TextInput(
            label="Appearance",
            placeholder="Describe their look, style, hair, features, etc.",
            style=discord.TextStyle.paragraph,
            max_length=1000,
            required=True,
        )
        self.character_backstory = discord.ui.TextInput(
            label="Backstory",
            placeholder="Describe their history, family, struggles, goals, etc.",
            style=discord.TextStyle.paragraph,
            max_length=2000,
            required=True,
        )
...
"
        ...
        await interaction.response.send_message(embed=embed("Character Created", text))
...
class CharacterGenreSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        modal = CharacterCreateModal(self.values)
...
@character.command(name="create", description="Create a character using a genre dropdown and form")
...
async def character_profile(interaction: discord.Interaction, character_id_or_name: str):
    ...
    text = (
        ...
        f"Genres: {item.get('genre', ', '.join(item.get('genres', [])) if item.get('genres') else 'N/A')}\n\n"
